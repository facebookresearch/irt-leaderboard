...
NOTE: This script has been further modified to be more customized to the leaderboard project.
...
class JsonApiClientWithRetry(JsonApiClient):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__wait_seconds = kwargs.pop("wait_seconds", 10)
...
class MimicDatasetsSchema(Schema):
    old = fields.String(required=True, metadata="Old dataset ID")
    new = fields.String(required=True, metadata="New dataset ID")
...
class ConfigSchema(Schema):
    ...
    mimic_datasets = fields.Nested(MimicDatasetsSchema, required=False)
...
class Competition(object):
    ...
    def __init__(self, config_path, output_path, leaderboard_only):
        ...
        auth = AuthHelper(
            ...
            or os.environ.get("CODALAB_USERNAME")
            or input("Username: "),
            self.config.get("password")
            or os.environ.get("CODALAB_PASSWORD")
...
def list_submissions(
    config_file: str, output_path: str, verbose: bool = False, leaderboard_only: bool = False,
):
    logging.basicConfig(
        format="%(message)s",
    ...
    comp = Competition(config_file, output_path, False)
    (submissions, num_total_submissions, num_period_submissions,) = comp.collect_submissions()
    write_json(
        output_path,
        {
    ...
    comp = Competition(config_file, output_path, leaderboard_only)
...
def cl_mimic(
    *,
    ...
    submit_id = submit_bundle["uuid"]
    args = ["cl", "mimic", "--depth", str(depth), "-v"]
    if dry_run:
        command = (f"cl mimic --dry-run --depth=5 -v {old_dataset} {submit_id} {new_dataset}",)
        args.append("--dry-run")
    else:
        command = (f"cl mimic --depth=5 -v {old_dataset} {submit_id} {new_dataset}",)
    args.extend(["--worksheet-spec", worksheet_id])
    args.extend(["--tags", tags])
    args.extend([old_dataset, submit_id, new_dataset])
    command = " ".join(args)
...
def mimic(
    ...
    dry_run: bool = False,
    preview: bool = False,
    error_file: str = "data/cl-errors.json",
    limit: int = -1,
    skip: int = 0,
    timeout: int = 30,
):
    common.URLOPEN_TIMEOUT_SECONDS = timeout
    ...
    comp = Competition(config_file, "/tmp/cl-out.json", leaderboard_only)
    # comp.ensure_log_worksheet_private()
    old_dataset_id = comp.config["mimic_datasets"]["old"]
    new_dataset_id = comp.config["mimic_datasets"]["new"]
    worksheet = comp.config["log_worksheet_uuid"]
    tags = comp.config["predict"]["tag"]
    depth = comp.config["predict"]["depth"]
    failed = []
    if not comp.leaderboard_only:
        submissions = submissions[skip:]
        if limit != -1:
            logging.info("Limiting number of bundle runs to %s", limit)
            submissions = submissions[:limit]
        for idx, submit_bundle in track(
            list(enumerate(submissions)), description="Submitting Bundles"
        ):
            try:
                predict_bundle_name = "{owner[user_name]}-{metadata[name]}-mlqa-predict".format(
                    **submit_bundle
                )
                if preview:
                    logger.info("Preview: idx=%s name=%s", skip + idx, predict_bundle_name)
                else:
                    logging.info("Mimic Bundle: idx=%s name=%s", skip + idx, predict_bundle_name)
                    metadata = {"tags": [tags], "request_time": None}
                    metadata.update(comp.config["predict"]["metadata"])
                    mimic_out = mimic_bundles(
                        client=comp.client,
                        old_inputs=[old_dataset_id],
                        old_output=submit_bundle["uuid"],
                        new_inputs=[new_dataset_id],
                        new_output_name=predict_bundle_name,
                        worksheet_uuid=worksheet,
                        depth=depth,
                        shadow=False,
                        skip_prelude=True,
                        metadata_override=metadata,
                    ...
                    mimicked = None
                    for old_info, new_info in mimic_out:
                        if old_info["uuid"] == submit_bundle["uuid"]:
                            mimicked = new_info
                    if mimicked is None:
                        logger.info(
                ...
                logging.warning("Bundle missing: %s, skipping", submit_bundle["uuid"])
    if not preview:
        write_json(error_file, {"failed_bundles": failed})


@cli.command()
def batch(
    config_file: str, output_path: str, leaderboard_only: bool = False, verbose: bool = False,
):
    logging.basicConfig(
        format="%(message)s",
        level=(logging.DEBUG if verbose else logging.INFO),
