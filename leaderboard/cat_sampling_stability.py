...
def cached_irt(irt_type: str) -> IrtParsed:
    if irt_type in CACHE_IRT:
        ...
        irt_model = IrtParsed.from_irt_file(
            Path(conf["irt"]["squad"]["dev"]["pyro"][irt_type]["subject_heldout"])
            / "parameters.json"
        )
...
class InfoSampler(Sampler):
    ...
    def sample(self, n_items: int):
        if n_items < len(self._chosen_items):
            ...
            subject_skills = {}
            thetas = seq(self._subject_ids).map(self.estimate_theta).list()
...
class Simulation:
    def __init__(self, max_size: int = 6000, step_size: int = 25, n_trials: int = 10):
        ...
        irt_model = "2PL"
        self._high_disc_sampler = IrtSampler(fold="dev", param_key="high_disc", irt_model=irt_model)
        self._low_disc_sampler = IrtSampler(fold="dev", param_key="low_disc", irt_model=irt_model)
        self._zero_disc_sampler = IrtSampler(fold="dev", param_key="zero_disc", irt_model=irt_model)

        self._high_diff_sampler = IrtSampler(fold="dev", param_key="high_diff", irt_model=irt_model)
        self._low_diff_sampler = IrtSampler(fold="dev", param_key="low_diff", irt_model=irt_model)
        self._zero_diff_sampler = IrtSampler(fold="dev", param_key="zero_diff", irt_model=irt_model)

        self._high_disc_diff_sampler = IrtSampler(
            fold="dev", param_key="high_disc_diff", irt_model=irt_model
        )
        ...
        self._dev_subjects = []
        self._test_subjects = []
        self._heldout_subjects = set(
            read_json(DATA_ROOT / conf["stability"]["dev_subject_splits"])["heldout"]
        )
        for test_id in unfiltered_test_subjects:
            if test_id in test_to_dev:
                dev_id = test_to_dev[test_id]
                if dev_id in self._heldout_subjects:
                    self._dev_subjects.append(test_to_dev[test_id])
                    self._test_subjects.append(test_id)

        log.info(f"N Dev Subjects: {len(self._dev_subjects)}")
        log.info(f"N Test Subjects: {len(self._test_subjects)}")
        log.info(f"N Heldout Subjects: {len(self._heldout_subjects)}")
    ...
    def run(self):
        log.info("Running random sampling")
        random_df = self.create_random_df()
        log.info("Running irt sampling")
        irt_df = self.create_irt_df()
        log.info("Running info sampling")
        info_df = self.create_info_df()
        output_dir = Path(conf["stability"]["cat_sampling"])
        random_df.to_json(safe_file(output_dir / "random_df.json"))
        irt_df.to_json(safe_file(output_dir / "irt_df.json"))
        info_df.to_json(safe_file(output_dir / "info_df.json"))
